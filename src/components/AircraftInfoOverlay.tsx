...
import {
  ...
  getRotation,
  getStatusText,
} from "../helpers/aircraftDataFunctions.js";

import {
  type IAircraftTrack,
  type resolvePositionSource,
  resolveCategory,
...
const AircraftInfoOverlay: React.FC<Props> = (props) => {
  ...
  useEffect(() => {
    ...
    if (!props.selectedAircraft || !stateVector) {
      setLastPositionPastSeconds(0);
      return;
    }
    setLastPositionPastSeconds(0);

    const lastPositionSeconds = stateVector.time_position
      ? stateVector.time_position
      : Math.floor(Date.now() / 1000);

    setLastPositionPastSeconds(
      Math.floor(Date.now() / 1000) - lastPositionSeconds
    );

    updateIntverlIDRef.current = window.setInterval(() => {
    ...
    props.selectedAircraft?.stateVector?.icao24,
    props.selectedAircraft?.stateVector?.last_contact
  ]);

  const renderHeader = () => {
    if (!props.selectedAircraft) return undefined;

    if (!props.selectedAircraft.stateVector) return undefined;
      ...
      <React.Fragment>
        ...
        >
          ...
          <IconButton
            aria-label="close"
            onClick={() => {
              if (!props.selectedAircraft) return;
  ...
  const renderFlightData = () => {
    ...
    if (stateVector.last_contact !== null) {
      ...
      lastContactTime = new Intl.DateTimeFormat("de-CH", options).format(
        lastContactDate,
      );
    }

    // Get altitude
    const barometricAltitude = stateVector.baro_altitude
      ? stateVector.baro_altitude
      : 0;
    const geometricAltitude = stateVector.geo_altitude
      ? stateVector.geo_altitude
      : 0;
    var altitude = stateVector.geo_altitude;
    if (altitude === null || altitude < 0) altitude = stateVector.baro_altitude;
    if (altitude === null || altitude < 0) altitude = 0;
    ...
    return (
      ...
      >
        ...
        <Box sx={textContainerStyle}>
          <Typography variant="body2">{"Last position update"}</Typography>
          <Typography variant="body1">
        ...
        <Box sx={textContainerStyle}>
          <Typography variant="body2">{"Barometric altitude"}</Typography>
          <Typography variant="body1">
        ...
        <Box sx={textContainerStyle}>
          <Typography variant="body2">{"Geometric altitude"}</Typography>
          <Typography variant="body1">
        ...
        <Box sx={textContainerStyle}>
          <Typography variant="body2">{"Longitude / Latitude"}</Typography>
          <Typography variant="body1">
        ...
        <Box sx={textContainerStyle}>
          <Typography variant="body2">{"Vertical rate"}</Typography>
          <Typography variant="body1">
        ...
        <Box sx={textContainerStyle}>
          <Typography variant="body2">{"ICAO24"}</Typography>
          <Typography variant="body1">{stateVector.icao24}</Typography>
        </Box>

        <Box sx={spaceStyle} />

        <Box sx={textContainerStyle}>
          <Typography variant="body2">{"Transpondercode [Squawk]"}</Typography>
          <Typography variant="body1">
